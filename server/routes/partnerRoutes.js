...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    // Auto-create KrishakJan partner in DB if first login
...
// PARTNER AUTH: GET /me/dashboard — Aggregated stats (Queries Referral + CommissionLedger + Inquiry + Order)
router.get(
  '/me/dashboard',
  authenticatePartnerToken,
  asyncHandler(async (req, res) => {
      ...
      try {
        ...
        dbReferrals = await Referral.find({
          $or: [
            ...
            {}, // Include all referrals for main partner dashboard
          ],
        }).lean().catch(() => []);
        ...
        dbCommissions = await CommissionLedger.find({
          $or: [
            { partnerId: { $in: targetIds } },
            {}, // Include all commissions for main partner dashboard
          ],
        }).lean().catch(() => []);
...
// PARTNER AUTH: GET /me/referrals — Referred Farmers List (Combines Referral + Inquiry + Live Store)
router.get(
  '/me/referrals',
  authenticatePartnerToken,
  asyncHandler(async (req, res) => {
    ...
    if (isDbConnected) {
      try {
        ...
        const referrals = await Referral.find({
          $or: [
            { partnerId: { $in: targetIds } },
            {}, // Return all referrals for partner view
          ],
        })
...
// PARTNER AUTH: GET /me/commissions — Commission Ledger List
router.get(
  '/me/commissions',
  authenticatePartnerToken,
  asyncHandler(async (req, res) => {
...
// PARTNER AUTH: PATCH /me/password
router.patch(
  '/me/password',
  authenticatePartnerToken,
  asyncHandler(async (req, res) => {
